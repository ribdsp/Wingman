...
// bucketTTL is how long an idle bucket is kept. Without it, the map grows once
// per distinct caller and never shrinks.
const bucketTTL = 10 * time.Minute

// KeyFunc decides what a request is limited against.
type KeyFunc func(*gin.Context) string

// KeyByIP limits per source address. Applied ahead of authentication, this is
// what keeps an unauthenticated flood from reaching the rest of the stack.
func KeyByIP(c *gin.Context) string { return "ip:" + c.ClientIP() }

// KeyByPrincipal limits per credential. Applied behind authentication, this is
// what stops one misbehaving bot — a retry loop that creates the same goal a
// thousand times — from consuming the whole service.
...
// RateLimit throttles requests sharing a key to rps per second with room for a
// burst.
//
// This is a safety net against a runaway caller, not a security control: on a
// self-hosted deployment behind a proxy every request may share one address, so
// the per-principal limiter is the one that discriminates.
...
// keyedLimiter is one token bucket per key, with idle buckets swept on use so
// there is no background goroutine to shut down.
...
// sweep drops buckets nobody has used for a while. It runs at most once per
// sweepGap so a busy service is not walking the whole map on every request.
