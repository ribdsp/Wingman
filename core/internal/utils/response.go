...
//
// It is a deliberate copy of the goal engine's package of the same name, not an
// import of it. The two services deploy independently and version independently;
// a shared module would couple them for the sake of about two hundred lines, and
// the coupling costs more than the duplication. The envelope itself must not
// diverge — a client should not be able to tell which service answered by the
// shape of the reply — so a change here is a change there.
package utils
